...
public:
  ...
  AirportCounter(){
    //cout << static_cast<size_t>(TAirport::Last_) << endl;
    for(auto i = 0; i < static_cast<size_t>(TAirport::Last_); ++i){
  ...
  void Insert(TAirport airport){
   // data[static_cast<size_t>(airport)].first = static_cast<TAirport>(airport);
    data[static_cast<size_t>(airport)].second++;
  }

  // удалить одно вхождение данного элемента
  void EraseOne(TAirport airport){
    //data[static_cast<size_t>(airport)].first = airport;
    data[static_cast<size_t>(airport)].second--;
  }
