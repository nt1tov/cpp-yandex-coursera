...
public:
  void Book(int64_t time, const string& hotel_name,
            int client_id, int room_count) { //O(LlogQ)
    current_time_ = time;
    hotels_[hotel_name].Book({time, client_id, room_count});
  }
  int ComputeClientCount(const string& hotel_name) {//O(LlogQ+W*logQ)
    return hotels_[hotel_name].ComputeClientCount(current_time_);
  }
  int ComputeRoomCount(const string& hotel_name) {//O(LlogQ+W*logQ)
...
private:
  ...
  public:
    ...
    int ComputeClientCount(int64_t current_time) { //O(W*logQ)
      RemoveOldBookings(current_time);
      return client_booking_counts_.size();
    }
    int ComputeRoomCount(int64_t current_time) {//O(W*logQ)
      RemoveOldBookings(current_time);
      return room_count_;
    }
  private:
    ...
    void RemoveOldBookings(int64_t current_time) { //O(W*logQ)
      while (
